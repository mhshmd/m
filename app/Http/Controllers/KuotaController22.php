...
class KuotaController extends Controller
{
    public function was(Request $request)
    {
        ...
        $telkomsel = new Menu(1, "Telkomsel",1,1,2);




        $pulsa = new Menu(2,"Pulsa",0,1,1);
        $masaAktif = new Menu(3,"Masa aktif Kartu",0,1,1);
        $tokenPLN = new Menu(4,"Token PLN\n",0,1,1);
        $keranjang = new Menu(5,"Keranjang belanja",0,1,1);

        $home->addMenu($kuota);
        $home->addMenu($pulsa);
        $home->addMenu($masaAktif);
        $home->addMenu($tokenPLN);
